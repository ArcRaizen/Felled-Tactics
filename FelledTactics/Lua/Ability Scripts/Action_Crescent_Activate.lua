...
local unit = nil
local caster = LuaUnit(level:GetAllyUnit(Source["x"], Source["y"]))
local damage = 0
...
for key, value in pairs(AreaOfEffect) do
	...
	if Rank == 1 then
		damage = unit:TakeDamage(10 + caster.Strength, 0)
	elseif Rank == 2 then
		damage = unit:TakeDamage(20 + caster.Strength, 0)
	elseif Rank == 3 then
		damage = unit:TakeDamage(30 + caster.Strength, 0)
	end
