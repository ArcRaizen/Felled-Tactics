...
hit = (combatant.Skill * 2) + (combatant.Agility * 0.5) + 60 --weapon accuracy
...
if isAttacker == true then
	combatManager:SetCombatParametersAttacker(physicalDamage, magicalDamage, hit, avoid, 2)
else
	combatManager:SetCombatParametersDefender(physicalDamage, magicalDamage, hit, avoid, 1)
end
